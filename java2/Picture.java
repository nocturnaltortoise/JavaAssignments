...
public class Picture{
	...
	public static void main(String[] args){
		...
		final int IMAGE_SIDE_LENGTH = (int)Math.sqrt(input.length());
		...
		renderColourImage(input, colourArray, SCREEN_SIZE, IMAGE_SIDE_LENGTH);
		renderSketch(colourArray, SKETCH_SCREEN_SIZE, IMAGE_SIDE_LENGTH);
		
	}

	private static void renderColourImage(String input, Colour[][] colourArray, final int SCREEN_SIZE, final int IMAGE_SIDE_LENGTH){
		...
		for(int y = 0; y < IMAGE_SIDE_LENGTH; y++){

			for(int x = 0; x < IMAGE_SIDE_LENGTH; x++){
				...
				int redValue = getRGB(colourArray[x][y])[0];
				int greenValue = getRGB(colourArray[x][y])[1];
				int blueValue = getRGB(colourArray[x][y])[2];
				//sets the colour for drawing based on the enum value for that pixel
				mainWindow.setColor(redValue, greenValue, blueValue);
	...
	private static void renderSketch(Colour[][] colourArray, final int SKETCH_SCREEN_SIZE, final int IMAGE_SIDE_LENGTH){
		 ...
		 * starts at 1 and ends at IMAGE_SIDE_LENGTH - 1 so it doesn't either go outside the range of the array
		 * or detect the edges of the screen. 
		 */
		for(int y = 1; y < IMAGE_SIDE_LENGTH - 1; y++){

			for(int x = 1; x < IMAGE_SIDE_LENGTH - 1; x++){
